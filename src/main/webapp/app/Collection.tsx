...
import { getCollection } from './shared/lib/algorand';
import { validArc3 } from './shared/lib/validator';
import { NFT } from './shared/lib/nft';
...
export function Collection(props: CollectionProps) {
  ...
  if (loaded) {
    if (collection.length > 0) {
        ...
        .map((nft: NFT) => {
          ...
          return (
            <Card className="content-collection-item" key={nft.id()} elevation={Elevation.TWO}>
              ...
              <AnchorButton icon={icon} minimal={true} href={'/nft/' + nft.id()}>
                <b>{nft.name()}</b>
              </AnchorButton>
            </Card>
          );
      ...
      nfts = [
        <h3 key="none">
          You dont have any ARC3 compliant NFTs on {conf[props.activeConf].network}, <a href="/mint">Mint one?</a>
        </h3>,
      ];
    }
  }
  return (
    <div className="container">
      <div className="content content-collection">{nfts}</div>
    </div>
  );
}
